...
class NCBI_search(object):
    def __init__(self, seq, savename, program='blastn', database='nt', desc=20, checkmax=20):
    ...
    def check_search(self):
        ...
        try:
            check = requests.post(url=self.url, data=check_params, timeout=15)
...
class batch_search(object):
    def __init__(self, seq_file, interval=5, loop_interval=180, test_mode=False):
        ...
        seq_find_regex = re.compile(r'\>.{0,3}[0-9]+.*\s*[ATCGU]{10,}')
        find_list = re.findall(seq_find_regex, file.read())
        file.close()

        seq_index_regex = re.compile(r'\>.{0,3}[0-9]+')
...
def main():
    ...
    try:
        ...
        if init_input == "SETTINGS":
            ...
            i_interval, i_loop_interval = params[:]
